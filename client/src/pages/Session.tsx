...
export default function SessionScreen() {
  ...
  const [voiceEnabled, setVoiceEnabled] = useState(false); // off by default in short session — user can toggle
  ...
  const { state: speakState, speak, stop: stopSpeaking } = useSpeak({
    ...
    rate: 0.92,
    onEnd: () => {
      if (voiceEnabled && !isEnding) startListening();
    },
  });
  ...
  useEffect(() => {
    ...
    const openingPrompt = getPrompt("situation");
  ...
  const handleSend = useCallback((textOverride?: string) => {
    ...
    if (nextIdx >= CBT_STAGES.length) {
      ...
      const finalMsgs = [...newMessages, closing];
      setMessages(finalMsgs);
      updateSession.mutate({ ...updates, summary, messages: JSON.stringify(finalMsgs) });
  ...
  }, [input, messages, stageIdx, sessionData, updateSession, stopListening, voiceEnabled, speak, startListening]);
  ...
  async function handleSave() {
    ...
    await updateSession.mutateAsync({
      ...
      cognitive_pattern: cogPattern || undefined,
      is_complete: true,
      follow_up_status: "not_done",
      next_time_prompt: sessionData.next_action
        ? `Did you manage to: ${sessionData.next_action}?`
        : undefined,
    });
    navigate("/");
  }
  ...
  const stageProgress = Math.min((stageIdx / CBT_STAGES.length) * 100, 100);
    ...
    <div className="min-h-dvh bg-background flex flex-col max-w-md mx-auto" data-testid="session-screen">
      ...
      <div className="flex items-center gap-3 px-5 pt-10 pb-3">
        <button
          ...
          onClick={() => { stopSpeaking(); stopListening(); navigate("/"); }}
          className="tap-target opacity-60 hover:opacity-100"
        >
          <ChevronLeft size={22} />
        </button>
        <div className="flex-1">
          <div className="font-semibold text-foreground" style={{ fontSize: "var(--text-sm)", fontFamily: "var(--font-display)" }}>
            Short session
          </div>
          <div className="text-muted-foreground" style={{ fontSize: "var(--text-xs)" }}>
            {STAGE_LABELS[CBT_STAGES[stageIdx] ?? "action"]}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-muted-foreground" style={{ fontSize: "var(--text-xs)" }}>
            {stageIdx + 1}/{CBT_STAGES.length}
          </span>
          {/* Voice toggle */}
          {!ttsUnsupported && (
            <button
      ...
      <div className="mx-5 mb-4 rounded-full overflow-hidden" style={{ height: "3px", background: "hsl(var(--muted))" }}>
        <div
          ...
          style={{ width: `${stageProgress}%`, background: "hsl(var(--session-accent))" }}
        />
      </div>
          ...
          <div key={i} className={`fade-up ${msg.role === "assistant" ? "self-start" : "self-end"} max-w-[85%]`}>
            {msg.role === "assistant" ? (
              <div className="bubble-assistant px-4 py-3">
                <p className="text-foreground" style={{ fontSize: "var(--text-base)", lineHeight: 1.6, whiteSpace: "pre-line" }}>
                  {msg.content}
                </p>
              </div>
            ) : (
      ...
      {showMeta && isEnding && (
        <div className="mx-5 mb-4 rounded-xl border border-border bg-card p-4 fade-up">
          <div className="mb-3">
            <div className="flex items-center gap-1.5 mb-2 text-muted-foreground" style={{ fontSize: "var(--text-xs)" }}>
              <Tag size={12} />
              Tags
            </div>
              ...
              {SESSION_TAGS.map((tag) => (
                <button
                  ...
                  style={{
                    ...
                    color: selectedTags.includes(tag) ? "hsl(var(--primary-foreground))" : "hsl(var(--muted-foreground))",
          ...
          <div>
            <div className="text-muted-foreground mb-1.5" style={{ fontSize: "var(--text-xs)" }}>
              Cognitive pattern (optional)
            </div>
            <Select onValueChange={setCogPattern}>
              <SelectTrigger data-testid="select-cog-pattern" className="text-sm">
      ...
      {!isEnding ? (
        <div className="px-5 pb-10 pt-2">
          <div
            className="rounded-2xl border flex items-end gap-3 p-3 transition-colors"
            style={{
              borderColor: isListening ? "hsl(var(--primary) / 0.5)" : "hsl(var(--border))",
              background: "hsl(var(--card))",
            }}
          >
            <textarea
              data-testid="session-input"
              className="flex-1 bg-transparent outline-none resize-none text-foreground"
              style={{ fontSize: "var(--text-base)", minHeight: "44px", maxHeight: "160px", lineHeight: 1.5 }}
            ...
            {!micUnsupported && (
              <button
                ...
                className="tap-target rounded-xl flex items-center justify-center transition-all active:scale-95 relative"
                style={{
                  width: "44px",
                  height: "44px",
                  background: isListening ? "hsl(var(--primary))" : "hsl(var(--muted))",
                }}
                aria-label={isListening ? "Stop" : "Speak"}
              >
                {isListening && (
                  <span className="absolute inset-0 rounded-xl" style={{ background: "hsl(var(--primary) / 0.25)", animation: "ping 1.2s cubic-bezier(0,0,0.2,1) infinite" }} />
                )}
                {isListening
                  ? <MicOff size={18} className="text-primary-foreground relative" />
                  : <Mic size={18} className="text-muted-foreground relative" />
                }
              </button>
            )}
            ...
            <button
              ...
              onClick={() => handleSend()}
              className="tap-target rounded-xl flex items-center justify-center transition-all active:scale-95"
              style={{
                width: "44px",
                height: "44px",
